import {
  ...
  writeContract,
  yParityToV
} from "./chunk-UN7NFRCU.js";
import {
  sha256,
  sha384,
  sha512
} from "./chunk-EMHK2SLC.js";
import {
  BaseError,
  BaseError2,
  FeeCapTooHighError,
  ...
  trimLeft,
  validate
} from "./chunk-KH5AC3NK.js";
import "./chunk-DV5HPU5B.js";
import {
  __export
...
function verify2(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex2(publicKey).substring(2), { lowS: true, ...hash3 ? { prehash: true } : {} });
}
...
function verify3(options) {
  ...
  if (rpId !== void 0) {
    ...
    const expectedRpIdHash = sha2563(fromString(rpId), { as: "Bytes" });
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "secp256k1") {
    const secp256k1 = envelope;
    return concat(toHex4(secp256k1.signature), options.magic ? magicBytes : "0x");
  }
  if (type2 === "p256") {
    const p2563 = envelope;
    return concat(serializedP256Type, fromNumber(p2563.signature.r, { size: 32 }), fromNumber(p2563.signature.s, { size: 32 }), fromNumber(p2563.publicKey.x, { size: 32 }), fromNumber(p2563.publicKey.y, { size: 32 }), fromNumber(p2563.prehash ? 1 : 0, { size: 1 }), options.magic ? magicBytes : "0x");
  }
  if (type2 === "webAuthn") {
    const webauthn = envelope;
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload3(from8(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload3(from8(envelope)),
...
async function serializeTempo(transaction, sig) {
  ...
  if (signature && typeof transaction.feePayer === "object") {
    ...
    const hash3 = TxEnvelopeTempo_exports.getFeePayerSignPayload(tx, {
      sender
    });
...
var chainConfig5 = {
  ...
  async verifyHash(client, parameters) {
    const { address, hash: hash3, signature, mode: mode2 } = parameters;
    const envelope = (() => {
    ...
    if (envelope) {
      if (envelope?.type === "keychain" && mode2 === "allowAccessKey") {
        ...
        if (keyInfo.expiry <= BigInt(Math.floor(Date.now() / 1e3)))
          return false;
        const innerPayload = envelope.version === "v2" ? keccak256(concat("0x04", hash3, address)) : hash3;
