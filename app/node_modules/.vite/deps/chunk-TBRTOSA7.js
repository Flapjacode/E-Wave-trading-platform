...
var ConnectorUnavailableReconnectingError = class extends BaseError2 {
...
async function verifyMessage3(config, parameters) {
  const { chainId, ...rest } = parameters;
  const client = config.getClient({ chainId });
  const action = getAction2(client, verifyMessage, "verifyMessage");
  return action(rest);
}
...
async function verifyTypedData3(config, parameters) {
  const { chainId, ...rest } = parameters;
  const client = config.getClient({ chainId });
  const action = getAction2(client, verifyTypedData, "verifyTypedData");
  return action(rest);
}
...
//# sourceMappingURL=chunk-TBRTOSA7.js.map
